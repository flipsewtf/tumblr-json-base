...
(function () {
    ...
    function onMove(event) {
        ...
        if (left + rect.width + PADDING > (isNowrap ? viewportWidth : window.scrollX + viewportWidth)) {
            const gutterOffset = isNowrap ? SCROLLBAR_WIDTH : 0;
            left = (isNowrap ? 0 : window.scrollX) + viewportWidth - gutterOffset - rect.width - PADDING;
        }
...
(function () {
    ...
    const units = [
        ...
        { limit: 3600, label: 'm', divisor: 60 },
        { limit: 86400, label: 'h', divisor: 3600 },
        { limit: 604800, label: 'd', divisor: 86400 },
        { limit: 2620800, label: 'w', divisor: 604800 },
        { limit: 31449600, label: 'mo', divisor: 2620800 },
        { limit: Infinity, label: 'y', divisor: 31449600 },
    ];
    ...
    elements.forEach((el) => {
        ...
        for (const unit of units) {
            if (diff < unit.limit) {
                el.textContent = unit.label === 'now' ? 'now' : Math.floor(diff / unit.divisor) + unit.label;
                break;
            }
        }
    });
...
document.querySelectorAll('.notecount').forEach((el) => {
    ...
    el.textContent = el.classList.contains('has-label') ? `${formatNumber(count)}` : formatNumber(count);
});
